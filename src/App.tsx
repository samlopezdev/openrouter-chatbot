...
function App() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (loading) return;
    if (!hasText && !(isVisionModel && hasImage)) return;
    ...
    try {
      ...
      const messageContent = parts.length
        ? parts
      ...
      let reply: any = choice?.message?.content;
      ...
      if (!reply || (typeof reply === "string" && reply.trim() === "")) {
        const backendError =
          data?.error?.message || "No response from model UwU (empty content)";
      ...
      setError(
        err?.message || "Something went wrong. Check API key and try again.",
      );
    } finally {
      setLoading(false);
    }
